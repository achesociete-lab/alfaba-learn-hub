...
interface LessonScreensProps { lesson: Lesson; onComplete: () => void; }
  ...
  return (
    <motion.div {...slideIn} className="flex flex-col items-center justify-center min-h-[55vh] text-center space-y-6 px-4">
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3, type: "spring", stiffness: 200 }}
        className="h-24 w-24 rounded-full gradient-emerald flex items-center justify-center text-4xl">{lesson.icon}</motion.div>
      <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
        className="text-2xl sm:text-3xl font-bold text-foreground">Bienvenue dans la Leçon {lesson.id}</motion.h2>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="space-y-1">
...
function TheoryScreen({ section, onNext, isLast }: { section: TheorySection; onNext: () => void; isLast: boolean }) {
  ...
  const handleLetterClick = useCallback((idx: number, letter: string) => {
    speak(letter); setClickedLetters(prev => new Set(prev).add(idx));
  }, [speak]);

  return (
    <motion.div {...slideIn} className="space-y-6">
      <div className="text-center"><h2 className="text-xl sm:text-2xl font-bold text-foreground mb-2">{section.title}</h2></div>
      ...
      {section.tip && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          <Sparkles className="h-4 w-4 text-primary mt-0.5 shrink-0" /><p className="text-sm text-primary">{section.tip}</p>
        </motion.div>
      )}
      {section.letterGrid && (
        <div className="space-y-2">
          {!canProceed && <p className="text-xs text-center text-muted-foreground">Clique sur au moins {minClicks - clickedLetters.size} lettre{minClicks - clickedLetters.size > 1 ? "s" : ""} pour continuer</p>}
          <div className="grid grid-cols-4 sm:grid-cols-7 gap-2.5 max-w-2xl mx-auto" dir="rtl">
            {section.letterGrid.map((l, i) => {
              ...
              return (
                <motion.button key={i} initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }}
                  onClick={() => handleLetterClick(i, l.letter)}
                  className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all cursor-pointer group ${isClicked ? "border-primary/50 bg-primary/10 ring-1 ring-primary/30" : "border-border bg-card hover:border-primary/40 hover:bg-primary/5"}`}>
                  <span className={`font-arabic text-2xl sm:text-3xl transition-colors ${isClicked ? "text-primary" : "text-foreground group-hover:text-primary"}`}>{l.letter}</span>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="overflow-x-auto rounded-xl border border-border bg-card">
          <table className="w-full text-center">
            <thead><tr className="border-b border-border bg-muted/50">
              {["Lettre","Seule","Début","Milieu","Fin"].map(h => <th key={h} className="py-2.5 px-2 text-xs text-muted-foreground font-medium">{h}</th>)}
            </tr></thead>
            <tbody>
              {section.formsTable.map((row, i) => (
                <motion.tr key={i} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 + i * 0.08 }}
                  className="border-b border-border/50 hover:bg-muted/30 cursor-pointer" onClick={() => speak(row.isolated)}>
                  <td className="py-2.5 text-xs text-muted-foreground">{row.name}</td>
                  {[row.isolated, row.initial, row.medial, row.final].map((v, j) => <td key={j} className="py-2.5 font-arabic text-xl text-foreground">{v}</td>)}
                </motion.tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="flex justify-center pt-2">
        <Button size="lg" onClick={onNext} disabled={!canProceed} className="gap-2 gradient-emerald border-0 text-primary-foreground">
...
// BUG 1 FIX: position persistée via usePersistentState (clé scopée par user + leçon).
export default function LessonScreens({ lesson, onComplete }: LessonScreensProps) {
  const { user } = useAuth();
  const [screen, setScreen] = usePersistentState<number>(userScopedKey(user?.id, `n1:lesson:${lesson.id}:theory:screen`), 0);
  const theorySections = lesson.theory || [];
  const totalScreens = 1 + theorySections.length;
  const progressPct = ((screen + 1) / totalScreens) * 100;
  const handleComplete = () => { setScreen(0); onComplete(); };

  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>Étape {screen + 1}/{totalScreens}</span><span>{Math.round(progressPct)}%</span>
        </div>
      ...
      <AnimatePresence mode="wait">
        {screen === 0
          ...
          : <TheoryScreen key={`theory-${screen}`} section={theorySections[screen - 1]}
              onNext={() => screen >= totalScreens - 1 ? handleComplete() : setScreen(s => s + 1)}
