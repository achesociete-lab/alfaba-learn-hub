...
function EvaluateTab({ toast }: { toast: ReturnType<typeof useToast>["toast"] }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: profs } = await supabase
        ...
        .in("user_id", ids);
      setStudents(
        (profs || []).map((p: any) => ({ ...p, student_id: p.user_id }))
      );
    })();
  }, []);

  useEffect(() => {
    if (!studentId) { setSessions([]); setSessionId(""); return; }
    (async () => {
      const { data } = await supabase
        .from("hifz_sessions")
        .select("*")
  ...
  const submit = async () => {
    if (!studentId || !sessionId) {
      toast({ title: "Champs manquants", description: "Sélectionnez l'élève et la session.", variant: "destructive" });
      return;
    }
    const valid = evals.filter((e) => e.status);
    if (!valid.length) {
      toast({ title: "Aucune évaluation", description: "Ajoutez au moins un hizb évalué.", variant: "destructive" });
      return;
    }
    setSaving(true);
    const { data: u } = await supabase.auth.getUser();
    ...
    setSaving(false);
    toast({ title: "Évaluation enregistrée", description: "L'élève sera notifié par email." });
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="font-semibold text-emerald-800">Hizb évalués</h4>
            ...
            <CardContent className="pt-4 space-y-3">
                  ...
                  <Select value={String(e.hizb_number)} onValueChange={(v) => updateEval(i, { hizb_number: +v })}>
                    ...
                    <SelectContent className="max-h-72">
                      {Array.from({ length: 60 }).map((_, k) => (
                        <SelectItem key={k + 1} value={String(k + 1)}>{k + 1}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                {evals.length > 1 && (
                  <Button size="sm" variant="ghost" onClick={() => removeEval(i)}>
                    <Trash2 className="h-4 w-4 text-red-600" />
                  </Button>
                )}
              </div>
              ...
              <div className="flex gap-2">
                <Button
                  size="sm"
                  type="button"
                  onClick={() => updateEval(i, { status: "valide" })}
                  ...
                  variant={e.status === "valide" ? "default" : "outline"}
                >
                  ✅ Validé
                </Button>
                <Button
                ...
                <div className="flex flex-wrap gap-2">
                  {NIVEAUX.map((n) => (
                    <Button
                      ...
                      className={e.niveau === n.value ? n.color : ""}
                    >
                      {n.label}
                    </Button>
                  ))}
              ...
              <Textarea
                ...
                value={e.notes}
                onChange={(ev) => updateEval(i, { notes: ev.target.value })}
                className="bg-white"
                rows={2}
              />
            </CardContent>
