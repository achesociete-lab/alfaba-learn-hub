...
ALTER TABLE public.presentiel_courses
  ADD COLUMN IF NOT EXISTS lesson_photos jsonb NOT NULL DEFAULT '[]'::jsonb;

ALTER TABLE public.presentiel_submissions
  ADD COLUMN IF NOT EXISTS photo_urls jsonb NOT NULL DEFAULT '[]'::jsonb,
  ADD COLUMN IF NOT EXISTS seen_by_student boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  -- Si nom/prénom toujours vides, splitter "name" ou "full_name"
  ...
  INSERT INTO public.profiles (user_id, first_name, last_name, level, avatar_url)
  VALUES (
    NEW.id,
    COALESCE(v_first, ''),
    COALESCE(v_last, ''),
