...
interface AdminNewStudentSignupProps {
  studentName?: string
  studentEmail?: string
  studentLevel?: string
  ccEmail?: string
...
const AdminNewStudentSignupEmail = ({ studentName, studentEmail, studentLevel, ccEmail }: AdminNewStudentSignupProps) => (
  <Html lang="fr" dir="ltr">
    <Head />
    <Preview>Nouvelle inscription d'élève sur {SITE_NAME}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Heading style={h1}>🎓 Nouvelle inscription d'élève</Heading>
        <Text style={text}>
          Un nouvel élève vient de s'inscrire sur {SITE_NAME}. Voici ses informations :
        </Text>
        <Section style={infoBox}>
          <Text style={infoLine}><strong>Nom et Prénom :</strong> {studentName || 'Non renseigné'}</Text>
          <Text style={infoLine}><strong>Email :</strong> {studentEmail || 'Non renseigné'}</Text>
          <Text style={infoLine}><strong>Niveau :</strong> {studentLevel || 'Non spécifié'}</Text>
        </Section>
        <Text style={text}>
          Vous pouvez accéder à l'espace admin pour valider l'inscription et gérer les accès.
        </Text>
...
export const template = {
  component: AdminNewStudentSignupEmail,
  subject: 'Nouvelle inscription d\'élève',
  displayName: 'Nouvelle inscription d\'élève',
  previewData: { 
    studentName: 'Ahmed Benali', 
    studentEmail: 'ahmed@example.com',
    studentLevel: 'Niveau 1',
  },
